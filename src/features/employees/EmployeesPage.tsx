  ...
  return (
    <PageShell>
      <div className="page-container">
        ...
        {isLoading ? <p className="py-12 text-center text-muted-foreground">Loading employees…</p> : isError ? <div className="rounded-xl border border-destructive/30 py-12 text-center"><p>Employees could not be loaded.</p><Button className="mt-3" variant="outline" onClick={() => refetch()}>Try again</Button></div> : <DataTable columns={columns} data={employees as Employee[]} searchKey="firstName" />}
      </div>
      <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open && !createEmployee.isPending) setForm(emptyForm) }}>
        <DialogContent>
          <DialogHeader><DialogTitle>Add employee</DialogTitle><DialogDescription>Create an employee profile. Required fields are marked with an asterisk.</DialogDescription></DialogHeader>
